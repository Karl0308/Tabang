...
const Assets = () => {
    ...
    const fetchData = () => {
        setIsLoading(true);
        axiosInstance.get(APIURLS.asset.getAssetTags)
            .then(res => res.data)
            .then(
                (result) => {
                    setData(result);
        ...
        <div className="container">
            ...
            <div className="overflow-x-auto mt-4">
                <table className="min-w-full">
                    <thead>
                        <tr>
                            <th className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer" onClick={() => handleOrder('code')}>
                            ...
                            <th className="px-6 py-3 bg-gray-100 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer" onClick={() => handleOrder('equipment')}>
                    ...
                    <tbody>
                        {currentItems.map((item, index) => (
                            <tr key={item.id} className={`hover:bg-gray-200 ${index !== currentItems.length - 1 ? 'border-b border-gray-300' : ''}`}>
                                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 text-left">{item.code}</td>
                                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 text-left">{item.name}</td>
                                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 text-left">{item.branch}</td>
                                <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500 text-left">{item.equipment}</td>
