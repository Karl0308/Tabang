...
const ViewTicket = ({ openModal, onHide, _ticketNum, _ticket, _users, _branches, _ticketChanges }) => {
    ...
    const handleAssetOnChange = (selectedOption) => {
        // let isAdd = selectedOption.length > ticketAssets.length;
        // setTicketAssets(selectedOption);
        // alert(isAdd);
        //  saveTicketProp("ticketAssets", JSON.stringify(selectedOption));

        const isAdd = selectedOption.length > ticketAssets.length;
    
        const difference = isAdd
            ? selectedOption.filter(option => !ticketAssets.includes(option))
            : ticketAssets.filter(option => !selectedOption.includes(option));
        
        console.log(isAdd ? "Added:" : "Removed:", difference);
    
        setTicketAssets(selectedOption);
        if(isAdd){
            // saveTicketProp("ticketAssets", JSON.stringify(selectedOption));
            saveTicketProp("ticketAssets", JSON.stringify(difference));
        }
        else{

            // saveTicketProp("ticketAssetsRemove", JSON.stringify(selectedOption));
            saveTicketProp("ticketAssetsRemove", JSON.stringify(difference));
        }
    };
    ...
    const Statuses = () => {
        ...
        return (
            <>
                (isSmallScreen ? (
                        <>
                            <option style={{ paddingLeft: "0px", paddingRight: "0px", textAlign: "center", color: "#F5A314", fontSize: "3vw", letterSpacing: ".5px", paddingBottom: "10px" }} value={1} selected={ticket.status == 1 ? true : false}>Pending</option>
                            <option style={{ paddingLeft: "0px", paddingRight: "0px", textAlign: "center", color: "#67A8E3", fontSize: "3vw", letterSpacing: ".5px", paddingBottom: "10px" }} value={3} selected={ticket.status == 3 ? true : false}>Approved</option>
                            <option style={{ paddingLeft: "0px", paddingRight: "0px", textAlign: "center", color: "#10B981", fontSize: "3vw", letterSpacing: ".5px", paddingBottom: "10px" }} value={0} selected={ticket.status == 0 ? true : false}>Closed</option>
        ...
        return (
            <>
                ...
                <Modal show={openModal} onHide={hideModal}>
                    {/* <div style={{ position: "absolute", right: 0, display: isLoading ? "inline" : "none" }} > <LoadingSpinner /></div> */}
                            ...
                            <div style={{ padding: "6px" }} >
                                ...
                                <div>
                                    ...
                                    <Form.Select name={"documentTypeId"}
                                        style={{ resize: "none", fontSize: "2.5vw", color: "#223E58", fontWeight: "100", whiteSpace: 'pre-line', overflowWrap: 'break-word' }}
                        ...
                        <div style={{ flex: '40%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ flex: '1', border: '1px solid #DBD3D3', position: 'relative' }}>
                                ...
                                <div style={{ padding: "10px" }}>
                                    ...
                                    <div style={{ fontSize: "2.5VW", color: "#223E58", fontWeight: "100", backgroundColor:  ticket.status === 0 ?  setSeverityTimeStamp(ticket.timeStamp, new Date()) : setSeverity(ticket.calledIn, new Date()), padding: "8px", borderRadius: "8px" }}> {ticket.status === 0 ? formatTimeDifference(ticket.timeStamp, ticket.calledIn) : formatTimeDifference(new Date(), ticket.calledIn)}</div>
                                </div>
                        ...
                        <div style={{ flex: '20%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ flex: '1', border: '1px solid #DBD3D3', position: 'relative' }}>
                                ...
                                <div style={{ padding: "10px" }}>
                                    ...
                                    <div style={{ fontSize: "15px", color: "#223E58", fontWeight: "100", backgroundColor:  ticket.status === 0 ?  setSeverityTimeStamp(ticket.timeStamp, new Date()) : setSeverity(ticket.calledIn, new Date()), padding: "8px", borderRadius: "8px" }}>{ticket.status === 0 ? formatTimeDifference(ticket.timeStamp, ticket.calledIn) : formatTimeDifference(new Date(), ticket.calledIn)}</div>
                                </div>
                                ...
                                <PdfExportWithFormAndImage ticket={ticket} />
                            </div>
